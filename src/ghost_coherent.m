function [multipathHeatMap, ghostLocations, debugInfo] = ghost_coherent(params, turbinePositions, X, Y, turbineSegments, debugFlag)
...
%   debugInfo        : Structure with intermediate variables (if debugFlag true).
    ...
    for i = 1:N_turbines
        ...
        for j = 1:N_turbines
            ...
            % Implement coherent summation for the field contributions.
            total_E_ref = 0;  % Initialize complex field for coherent sum.
            sum_R1 = 0;
            for s_i = 1:size(segs_i, 1)
                ...
                % Calculate complex field contribution for this segment.
                E_seg = sqrt(P_ref) * exp(-1j*(2*pi/lambda)*R1);
                total_E_ref = total_E_ref + E_seg;
            end
            % Replace scalar power sum with coherent power (squared magnitude of summed field).
            total_P_ref = abs(total_E_ref)^2;
            avg_R1 = sum_R1 / size(segs_i,1);
            % Turbine i now acts as a source with coherent power total_P_ref.
            
            %% Stage 2: Turbine i midpoint → Turbine j segments
            % Again implement coherent summation over segments.
            total_E_ref_prime = 0;  % Initialize complex field for second bounce.
            sum_R2 = 0;
            for s_j = 1:size(segs_j, 1)
                ...
                % Here, note that we use the coherent power from stage 1.
                P_ref_prime = (total_P_ref * sigma_bj) / (4*pi * R2^2);
                % Compute complex field contribution for this segment in stage 2.
                E_seg_prime = sqrt(P_ref_prime) * exp(-1j*(2*pi/lambda)*R2);
                total_E_ref_prime = total_E_ref_prime + E_seg_prime;
            end
            % Replace scalar power sum with coherent power (squared magnitude of summed field) for stage 2.
            total_P_ref_prime = abs(total_E_ref_prime)^2;
            avg_R2 = sum_R2 / size(segs_j,1);
            % Turbine j now acts as a source with coherent power total_P_ref_prime.
            ...
            P_received_max = (total_P_ref_prime * lambda^2 * baseG_r) / ((4*pi)^3 * (sqrt(R1+R2+R3)).^4);
